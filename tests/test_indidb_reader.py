...
Adapted copy of indigo-mcp-lite's ``tests/test_indidb_reader.py``
(keep aligned, like the module itself). The only adaptation is the
``mock_indigo`` fixture: lite's conftest injects a MagicMock into
``sys.modules['indigo']``, whereas HI's conftest installs a strict
stub. ``IndiDbReader`` never does ``import indigo`` — the module is
injected via ``indigo_module`` — so a plain local MagicMock fixture
suffices and the strict stub stays untouched.

Fixture XML mirrors the real database shapes captured in lite issue
#38's schema recon: typed-XML dicts/vectors, anonymous nested
ActionGroup inside triggers/schedules, and every Action Class code
observed live.
"""
...
@pytest.fixture
def mock_indigo():
    """Stand-in for the injected ``indigo_module`` — a plain MagicMock,
    NOT a sys.modules override (see module docstring)."""
    return MagicMock(name="indigo")
