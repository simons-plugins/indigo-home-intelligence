...
Adapted copy of indigo-mcp-lite's
``tests/test_indidb_reader_semantics.py`` (keep aligned, like the
module itself). The only adaptation is the ``mock_indigo`` fixture:
HI's conftest installs a strict stub rather than a sys.modules
MagicMock, so the fixture is defined locally — same reasoning as
``test_indidb_reader.py``.
